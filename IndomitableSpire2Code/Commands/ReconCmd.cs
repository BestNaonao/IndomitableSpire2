...
{
    // 创建自定义的本地化提示框（稍后需要在 json 中配置该文本）
    public static LocString DrawPrompt = new("card_selection", "INDOMITABLESPIRE2-RECON_TO_HAND");
    public static LocString DiscardPrompt => new("card_selection", "INDOMITABLESPIRE2-RECON_TO_DISCARD");
    /// <summary>
    ...
    public static async Task Execute(PlayerChoiceContext choiceContext, Player player, int amount)
    {
        ...
        {
            // 如果你想触发“抽牌”相关的遗物，可以使用 PlayerCmd.DrawCard(card);
            // 如果你想悄无声息地放入手牌（类似全息影像），则使用 CardPileCmd.Add
